...
import { validateApiKey, AuthenticatedRequest } from './middleware/validateApiKey';
...
app.post(
  '/telemetry',
  validateApiKey,
  async (req: AuthenticatedRequest, res: express.Response): Promise<any> => {
    try {
      const { device } = req;
      ...
      const docRef = await db.collection('telemetry').add({
        ...
        ownerUid : device.ownerUid,   // necesario para que las Firestore Rules funcionen
